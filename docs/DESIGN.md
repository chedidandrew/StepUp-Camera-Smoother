...
flowchart TD
    ...
    F --> H["Camera perspective selected"]
...
Smoothness values from 0% through 100% scale `h`, preserving the original partial-correction behavior. Values above 100% keep `h` fully compensated and multiply only the recovery duration. This reaches 2x recovery at 200% without sending the camera below its pre-step height.
...
Minecraft 26.2 positions the camera in `Camera.alignWithEntity(float)`, which is called from `Camera.update(DeltaTracker)`. The mixin applies the correction immediately after `alignWithEntity` records whether the current view is detached.

This point was selected because:

- the entity-eye pivot and current perspective are available;
- rear and front third-person distance and wall collision are still calculated afterward from the corrected pivot;
...
The smoothing queue is cleared if the player or level instance changes, time moves backward, the camera focuses another entity, smoothing is disabled, the player leaves the ground, the player enters an excluded movement state, or the camera enters third person while third-person smoothing is disabled. The last resolved position is also compared before movement and camera sampling, so a same-player direct position correction or teleport clears the queue. This prevents a stale offset from surviving falls, respawn, dimension travel, spectator camera changes, mounts, unsupported perspective switches, or other discontinuities. Third-person smoothing is off by default and can be enabled for rear and front third-person views.

## Configuration screen

The optional Mod Menu screen exposes `smoothing_strength` as a 0% to 200% Smoothness slider and `smooth_third_person` as an on/off control. Zero leaves the original upward camera motion unchanged, 50% applies half of the visual correction, and 100% applies the full correction over the base recovery duration. Values through 200% retain full correction and extend recovery to at most twice that duration. The default is 150% Smoothness with third-person smoothing off. At 150%, the full visual correction is applied and recovery lasts 1.5 times the configured base duration.

The screen edits draft values. Cancel and Escape discard those drafts. Reset restores 150% Smoothness and Third Person Off without replacing recovery duration, easing, maximum lag, or debug settings. These draft defaults take effect only after selecting Done. Done writes a complete sanitized configuration through an atomic file replacement, publishes the new in-memory snapshot only after the write succeeds, and clears the active camera transition. The next eligible step therefore uses the new values immediately. A failed write leaves both the previous file and active configuration unchanged.

Configuration version 1 identifies the current configuration schema. A valid versionless alpha.2 file is migrated once, preserving explicitly saved preferences after sanitization. Missing settings use the current defaults. Existing saved settings are not overwritten by the 0.3.1 default changes. Malformed files and unsupported future configuration versions remain untouched; safe defaults are used for that launch.
